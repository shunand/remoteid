...
uint32_t com_hobbywing_esc_StatusMsg2_encode(struct com_hobbywing_esc_StatusMsg2* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, COM_HOBBYWING_ESC_STATUSMSG2_MAX_SIZE);
    _com_hobbywing_esc_StatusMsg2_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
bool com_hobbywing_esc_StatusMsg2_decode(const CanardRxTransfer* transfer, struct com_hobbywing_esc_StatusMsg2* msg) {
    uint32_t bit_ofs = 0;
    _com_hobbywing_esc_StatusMsg2_decode(transfer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct com_hobbywing_esc_StatusMsg2 sample_com_hobbywing_esc_StatusMsg2_msg(void) {
    struct com_hobbywing_esc_StatusMsg2 msg;

    msg.input_voltage = (int16_t)random_bitlen_signed_val(16);
    msg.current = (int16_t)random_bitlen_signed_val(16);
    msg.temperature = (uint8_t)random_bitlen_unsigned_val(8);
    return msg;
}
#endif
