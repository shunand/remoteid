...
uint32_t com_hobbywing_esc_GetEscID_encode(struct com_hobbywing_esc_GetEscID* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, COM_HOBBYWING_ESC_GETESCID_MAX_SIZE);
    _com_hobbywing_esc_GetEscID_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
bool com_hobbywing_esc_GetEscID_decode(const CanardRxTransfer* transfer, struct com_hobbywing_esc_GetEscID* msg) {
    uint32_t bit_ofs = 0;
    _com_hobbywing_esc_GetEscID_decode(transfer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct com_hobbywing_esc_GetEscID sample_com_hobbywing_esc_GetEscID_msg(void) {
    struct com_hobbywing_esc_GetEscID msg;

    msg.payload.len = (uint8_t)random_range_unsigned_val(0, 3);
