...
uint32_t com_hobbywing_esc_SetAngleResponse_encode(struct com_hobbywing_esc_SetAngleResponse* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, COM_HOBBYWING_ESC_SETANGLE_RESPONSE_MAX_SIZE);
    _com_hobbywing_esc_SetAngleResponse_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
bool com_hobbywing_esc_SetAngleResponse_decode(const CanardRxTransfer* transfer, struct com_hobbywing_esc_SetAngleResponse* msg) {
    uint32_t bit_ofs = 0;
    _com_hobbywing_esc_SetAngleResponse_decode(transfer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct com_hobbywing_esc_SetAngleResponse sample_com_hobbywing_esc_SetAngleResponse_msg(void) {
    struct com_hobbywing_esc_SetAngleResponse msg;

    msg.option = (uint8_t)random_bitlen_unsigned_val(8);
    msg.angle.len = (uint8_t)random_range_unsigned_val(0, 2);
    for (size_t i=0; i < msg.angle.len; i++) {
        msg.angle.data[i] = (uint16_t)random_bitlen_unsigned_val(16);
    }
    return msg;
}
#endif
