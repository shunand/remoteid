...
uint32_t com_hobbywing_esc_StatusMsg1_encode(struct com_hobbywing_esc_StatusMsg1* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, COM_HOBBYWING_ESC_STATUSMSG1_MAX_SIZE);
    _com_hobbywing_esc_StatusMsg1_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
bool com_hobbywing_esc_StatusMsg1_decode(const CanardRxTransfer* transfer, struct com_hobbywing_esc_StatusMsg1* msg) {
    uint32_t bit_ofs = 0;
    _com_hobbywing_esc_StatusMsg1_decode(transfer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
...
struct com_hobbywing_esc_StatusMsg1 sample_com_hobbywing_esc_StatusMsg1_msg(void) {
    struct com_hobbywing_esc_StatusMsg1 msg;

    msg.rpm = (uint16_t)random_bitlen_unsigned_val(16);
    msg.pwm = (uint16_t)random_bitlen_unsigned_val(16);
